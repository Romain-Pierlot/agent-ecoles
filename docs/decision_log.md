...
## S20.1 — Pipeline CI/CD test-gated : réorganisation des tests, tiering bloquant/informatif, déploiement automatique sur Render conditionné aux tests

**Décision** : Les 16 scripts de test à la racine du dépôt sont regroupés dans `tests/` (import en mode module `python3 -m tests.xxx` pour préserver les imports absolus existants, via un `pyproject.toml` minimal), à l'exception de `test_comparaison_noms.py`, interactif, déplacé vers `outils/comparaison_noms_interactif.py` pour ne jamais être ramassé par un runner automatisé. Deux workflows GitHub Actions : `tests.yml` (réutilisable, deux tiers : un job bloquant regroupant les tests déterministes/gratuits et le build frontend, un job informatif pour les 8 tests réseau/LLM, jamais bloquant vu leur coût et leur non-déterminisme) et `deploy.yml` (déclenche les Deploy Hooks Render backend/frontend indépendamment, chacun conditionné à la réussite de son propre job bloquant, avec un smoke test post-déploiement qui vérifie que `/health` sert bien le commit qu'on vient de pousser, via `RENDER_GIT_COMMIT`, pas seulement un 200 générique). Deux nouveaux tests ajoutés au tier bloquant : `test_api_chat.py` (premier test à passer par la vraie route HTTP `POST /chat`, les tests existants appelant le graphe LangGraph directement) et `test_journalisation_resilience.py` (vérifie que la panne de journalisation Supabase ne fait jamais échouer une réponse `/chat`).

**Pourquoi** : Le déploiement manuel avec rappel a été écarté au profit d'un gate automatique par les tests, pour éviter de dépendre de la mémoire humaine à chaque déploiement. Cinq des seize scripts de test existants ne faisaient jamais échouer le processus même en cas d'échec réel (impression du résultat sans `sys.exit`), corrigés pour porter un vrai code de sortie exploitable en CI. La base SQLite de production (~80 Mo) et ChromaDB étant volontairement exclues de git, deux stratégies de données coexistent selon ce que chaque test vérifie réellement : une fixture dédiée committée pour les tests dont les assertions sont comportementales (`test_sql_safety.py`), la vraie base restaurée depuis un cache GitHub Actions pour le seul test dont les assertions dépendent de comptages exacts sur les données nationales réelles (`test_zone_administrative.py`, DOM-TOM et Martinique).

**Rejeté** : Automatiser la reconstruction complète de la base depuis les CSV sources à chaque run CI (nécessiterait un téléchargeur automatisé qui n'existe pas encore) ; le rollback automatique via l'API Render (plus robuste mais un secret supplémentaire à protéger et plus de code à maintenir, gardé comme évolution possible une fois le rollback manuel éprouvé) ; inclure `npm run lint` dans le tier bloquant dès maintenant (64 erreurs pré-existantes découvertes en testant, dont 54 relèvent d'une vraie décision de design system non tranchée, cf. `journal_de_bord.md`).

**Conséquence** : `tests/` (17 fichiers), `outils/comparaison_noms_interactif.py`, `pyproject.toml`, `requirements-dev.txt`, `.github/workflows/tests.yml`, `.github/workflows/deploy.yml`, `data/fixtures/agent_ecoles_test.db`, `data/DATA_VERSION`. `config.py` : `DB_PATH` et `API_CORS_ORIGINS` surchargeables par variable d'environnement. `api/main.py` : `/health` retourne désormais le commit déployé. `deploy.yml` reste en déclenchement manuel (`workflow_dispatch`) tant que les services Render et leurs secrets n'existent pas.
